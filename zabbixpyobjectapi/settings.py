...
def _loadsettings(filename, parent=None):
    ...
    config = configparser.ConfigParser(interpolation=None, empty_lines_in_values=False,
                                       default_section="~#'[&INVALID&]'#~", dict_type=dict)
...
class Settings(object):
    ...
    def system(self):
        ...
        try:
            ...
            settingspath = "/etc/zabbixapi.conf" if __file__[:5] == "/usr/" else os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "zabbixapi.conf")
